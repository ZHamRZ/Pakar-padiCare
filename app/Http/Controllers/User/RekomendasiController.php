...
class RekomendasiController extends Controller
{
    ...
    public function previewCetak(Request $request)
    {
        ...
        // Filter by selected product codes from the preview page
        // null = param absent → show all; [] = param empty → show none
        $hasPupuk = $request->has('pupuk');
        $hasPestisida = $request->has('pestisida');

        if ($hasPupuk || $hasPestisida) {
            $pupukFilter = $hasPupuk ? array_filter(explode(',', $request->query('pupuk', ''))) : null;
            $pestisidaFilter = $hasPestisida ? array_filter(explode(',', $request->query('pestisida', ''))) : null;

            $hasilDiagnosa = $hasilDiagnosa->map(function (array $item) use ($pupukFilter, $pestisidaFilter) {
                $rekomendasi = $item['rekomendasi'];

                if ($pupukFilter !== null) {
                    $rekomendasi->detailPupuk = $rekomendasi->detailPupuk
                        ->filter(fn ($d) => in_array($d->pupuk->kode, $pupukFilter, true))
                        ->values();
                }

                if ($pestisidaFilter !== null) {
                    $rekomendasi->detailPestisida = $rekomendasi->detailPestisida
                        ->filter(fn ($d) => in_array($d->pestisida->kode, $pestisidaFilter, true))
                        ->values();
                }
        ...
        return $items->map(function (array $item) use ($type) {
            ...
            if ($type === 'pupuk') {
                $productData = array_merge($productData, [
                    ...
                    'satuan_dosis' => data_get($item, 'satuan_dosis', 'kg'),
                    'harga_per_unit' => data_get($item, 'harga_per_unit'),
                    'satuan_harga_qty' => data_get($item, 'satuan_harga_qty'),
                    'satuan_harga_unit' => data_get($item, 'satuan_harga_unit'),
            ...
            // Tambahkan field spesifik pestisida
                ...
                $productData = array_merge($productData, [
                    ...
                    'dosis_per_hektar' => data_get($item, 'dosis_per_hektar'),
                    'satuan_dosis' => data_get($item, 'satuan_dosis', 'ml'),
                    'harga_per_unit' => data_get($item, 'harga_per_unit'),
                    'satuan_harga_qty' => data_get($item, 'satuan_harga_qty'),
                    'satuan_harga_unit' => data_get($item, 'satuan_harga_unit'),
                    'efek_penggunaan' => data_get($item, 'efek_penggunaan'),
