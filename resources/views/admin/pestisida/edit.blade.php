        ...
        <form action="{{ route('admin.pestisida.update', $pestisida) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" min="0.01" step="0.01" name="dosis_per_hektar" value="{{ old('dosis_per_hektar', $pestisida->dosis_per_hektar) }}" class="form-control @error('dosis_per_hektar') is-invalid @enderror" required>
                ...
                <div class="col-md-2">
                    ...
                    <select name="satuan_dosis" class="form-select @error('satuan_dosis') is-invalid @enderror" required>
                        <option value="L" {{ old('satuan_dosis', $pestisida->satuan_dosis ?? 'L') === 'L' ? 'selected' : '' }}>Liter</option>
                        <option value="ml" {{ old('satuan_dosis', $pestisida->satuan_dosis ?? 'L') === 'ml' ? 'selected' : '' }}>ml</option>
                        <option value="kg" {{ old('satuan_dosis', $pestisida->satuan_dosis ?? 'L') === 'kg' ? 'selected' : '' }}>kg</option>
                        <option value="g" {{ old('satuan_dosis', $pestisida->satuan_dosis ?? 'L') === 'g' ? 'selected' : '' }}>g</option>
                    </select>
                    @error('satuan_dosis')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label">Harga Patokan (Rp)</label>
                    <input type="number" min="0" step="0.01" name="harga_per_unit" value="{{ old('harga_per_unit', $pestisida->harga_per_unit) }}" class="form-control @error('harga_per_unit') is-invalid @enderror" required>
                    @error('harga_per_unit')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-2">
                    <label class="form-label">Jumlah</label>
                    <input type="number" min="0.01" step="0.01" name="satuan_harga_qty" value="{{ old('satuan_harga_qty', $pestisida->satuan_harga_qty ?? 100) }}" class="form-control @error('satuan_harga_qty') is-invalid @enderror" required>
                ...
                <div class="col-md-2">
                    ...
                    <select name="satuan_harga_unit" class="form-select @error('satuan_harga_unit') is-invalid @enderror" required>
                        <option value="ml" {{ old('satuan_harga_unit', $pestisida->satuan_harga_unit ?? 'ml') === 'ml' ? 'selected' : '' }}>ml</option>
                        <option value="l" {{ old('satuan_harga_unit', $pestisida->satuan_harga_unit ?? 'ml') === 'l' ? 'selected' : '' }}>L</option>
                        <option value="g" {{ old('satuan_harga_unit', $pestisida->satuan_harga_unit ?? 'ml') === 'g' ? 'selected' : '' }}>g</option>
                        <option value="kg" {{ old('satuan_harga_unit', $pestisida->satuan_harga_unit ?? 'ml') === 'kg' ? 'selected' : '' }}>kg</option>
                    </select>
