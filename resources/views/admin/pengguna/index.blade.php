...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        <span>Daftar Pengguna Petani</span>
    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle" id="penggunaTable">
                ...
                <tbody>
                    ...
                    <tr data-nama="{{ $user->nama }}" data-username="{{ $user->username }}" data-email="{{ $user->email ?? '' }}">
                        <td data-label="Foto">
                        ...
                        <td data-label="Riwayat">{{ $user->rekomendasi_count }} riwayat</td>
                        <td data-label="Tanggal Daftar">{{ optional($user->created_at)->format('d M Y H:i') }}</td>
                        <td data-label="Aksi" class="text-end">
                            <div class="btn-group-vertical" role="group">
                                <button type="button" class="btn btn-sm btn-action btn-reset"
                                    data-user-id="{{ $user->id }}" data-user-name="{{ $user->nama }}"
                                    title="Reset Password" data-bs-toggle="tooltip" data-bs-placement="top">
                                    <i class="bi bi-key me-1"></i>Reset
                                </button>
                                <button type="button" class="btn btn-sm btn-action btn-delete-user-btn"
                                    data-user-id="{{ $user->id }}" data-user-name="{{ $user->nama }}"
                                    title="Hapus Pengguna" data-bs-toggle="tooltip" data-bs-placement="top">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('resetPasswordModal').addEventListener('hidden.bs.modal', () => onModalHidden('reset'));
    document.getElementById('deleteUserModal').addEventListener('hidden.bs.modal', () => onModalHidden('delete'));
    ...
    document.addEventListener('click', function(e) {
        ...
        const deleteBtn = e.target.closest('.btn-delete-user-btn');
        if (deleteBtn) {
            if (isProcessing) return;
    ...
    if (searchInput && table) {
        searchInput.addEventListener('input', function() {
            ...
            rows.forEach(row => {
                ...
                const match = nama.includes(query) || username.includes(query) || email.includes(query);
